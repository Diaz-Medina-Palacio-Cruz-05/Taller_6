...
public class InterfazLibreria extends JFrame
{
	...
	public void cargarArchivos(File archivo_categorias, File archivo_libros)
	{
		try
		{
			...
			String linea = "Se agregaron nuevas categorias: ";
			for (String key: libreria.nuevasCategorias.keySet())
			{
				String cantCat = (libreria.nuevasCategorias.get(key)).toString();
				linea += cantCat + " libro(s) para la categoria " + key + " ";
			}
	...
	{
		String viejoNombre = JOptionPane.showInputDialog("Digite el nombre de la categoria que quiere editar.");
		String nuevoNombre = JOptionPane.showInputDialog("Digite el nuevo nombre de la categoria.");
		try 
		{
			libreria.renombrarCategoria(viejoNombre, nuevoNombre);
		}
		catch(Exception ex)
		{
			JOptionPane.showMessageDialog(this, ex.getMessage(), "Ocurrio un Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	
	public void eliminarLibros() 
	{
		String autores = JOptionPane.showInputDialog("Ingrese los nombres de los autores de los libros que quiere eliminar.");
		try
		{
			String info = "Se eliminaron " + libreria.eliminarLibros(autores) + " libros.";
			JOptionPane.showMessageDialog(this, info, "Libros Eliminados", JOptionPane.INFORMATION_MESSAGE);
		}
		catch(Exception ex)
		{
			JOptionPane.showMessageDialog(this, ex.getMessage(), "Ocurrio un Error", JOptionPane.ERROR_MESSAGE);
